    ...
    >
      <section className="bg-gray-100 max-w-[42rem] border border-black/5 rounded-lg overflow-hidden sm:pr-8 relative sm:h-[20rem] hover:bg-gray-200 transition sm:group-even:pl-8 dark:bg-white/10 dark:hover:bg-white/20 dark:text-white">
        <div className="pt-4 pb-7 px-5 sm:pl-10 sm:pr-2 sm:pt-10 sm:max-w-[50%] flex flex-col h-full sm:group-even:ml-[18rem]">
          ...
          <ul className="flex flex-wrap mt-4 gap-2 sm:mt-auto pt-3 sm:group-even:justify-end">
            {tags.map((tag, index) => (
              <li
                key={index}
        ...
        <Image
          ...
          quality="95"
          className="absolute hidden sm:block top-[4.21rem] -right-40 w-[28.25rem] rounded-t-lg shadow-2xl transition  group-hover:scale-[1.04] group-hover:-translate-x-3 group-hover:translate-y-3 group-hover:-rotate-2 group-even:group-hover:translate-x-3 group-even:group-hover:translate-y-3 group-even:group-hover:rotate-2 group-even:right-[initial] group-even:-left-40 border border-black/10 dark:border-white/10"
        />
      </section>
    </motion.div>
  );
